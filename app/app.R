...
gages <- read_csv("gage_information_caravan700.csv") %>%
  add_row(GAGE_ID = "UserData1",GAGE_NAME = "User Data",country = "User_Data", 
          LAT = 0, LONG = 0, DRAINAGE_AREA_KM2 = 1)

# Define fitness function for genetic algorithm
fitness_function <- function(pars, input_data, PET, objfxn, snowWT) {
  modeloutput <- HBV(pars, input_data$Precip, input_data$Temp, PET, routing = 0)
  results <- bind_cols(modeloutput, Qobs = input_data$Qobs, Snow = input_data$Snow)
  ...
  if (is.na(snowNSE)) snowNSE <- 0
  if (snowWT > 0) NSE <- (NSE * (1 - snowWT)) + (snowNSE * snowWT)
  
  # print(NSE)  # Debugging: print the NSE value
  
  if (objfxn == "NSE") return(NSE) 
  
  NPEKG <- RNP(sim = results$q, obs = results$Qobs)
  snowNPEKG <- RNP(sim = justsnow$SWE, obs = justsnow$Snow)
  if (is.na(snowNPEKG)) snowNPEKG <- 0
  if (snowWT > 0) NPEKG <- (NPEKG * (1 - snowWT)) + (snowNPEKG * snowWT)
  ...
  sidebarLayout(
    sidebarPanel(width = 4,
                 ...
                 sliderInput("snowWT" , 
                             ...
                             value = 0,
                             step = 0.1),
                 numericInput("runs"  , "# of Runs for Mini-Monte Carlo", 
                              min = 100, 
                              max = 10000,
                              value = 100),
                 actionButton("runMC" , "Run Mini-Monte Carlo"),
                 h5(HTML("Click below to run Genetic Algorithm Parameterization")),
      ...
      tabsetPanel(type = "tabs",
                  ...
                  tabPanel("Upload Data",
                       ...
                       If everything went well your data will be shown below."),
                           h5("STEP 5: Enter a latitude for your site in the box below or in the Latitude field to the left"),
                           numericInput("new_lat", "Latitude of your Site", value = gages$LAT[gages$GAGE_NAME == gages$GAGE_NAME[1]]),
...
server <- function(input, output, session) {
  ...
  output$table1 <- renderPlotly({
    ...
    plot_ly(
      ...
      cells = list(
        values = rbind(
          c('NSE', 'NPE', 'Observed mean Q', 'Modeled mean Q'),
  ...
  observeEvent(input$Refresh,
               ...
               if(input$objfxn == "NPE")output$NSEplot <- renderPlot({
                 NSEparsdat <- read_csv("NSEpars.csv") %>% 
                   select(-MAXBAS) %>%
                   top_n(100, NPEKG) %>%
                   pivot_longer(cols = -c(NSE, NPEKG))
                 NSEparsdat %>%
                   ...
                   geom_point()+
                   facet_wrap(facets = vars(name), scales = "free_x", ncol = 3)+
                   ggtitle("Parameter values and NPE of top 100 runs")+
                   theme(text = element_text(size = 20))
               })
               })
  ...
  output$Map <- renderLeaflet({
    leaflet() %>% #addTiles() %>%
      ...
      addCircleMarkers(layerId = gages$GAGE_NAME[gages$GAGE_NAME != "User Data"], 
                       lat = gages$LAT[gages$GAGE_NAME != "User Data"], #input$lat_input, 
                       lng = gages$LONG[gages$GAGE_NAME != "User Data"],
                       radius = 4,
                       fillOpacity = 0.65) %>%
  ...
  #update latitude for user data
  observeEvent(input$new_lat, {
    # Update the 'lat' input based on the value in 'new_lat'
    updateNumericInput(session, "lat", value = input$new_lat)
  })
  ...
  observeEvent(input$runGA, {
    ga_results <- ga(
      type = "real-valued",
      fitness = function(pars) fitness_function(pars, input_data(), PET(), input$objfxn, input$snowWT),
      lower = c(40, 1, 0.3, 0.4, -1.5, 1, 0.05, 0.01, 0.001, 0, 0),
      upper = c(400, 6, 1, 1.2, 1.2, 8, 0.5, 0.3, 0.15, 70, 4),
      popSize = 100, #input$runs,
      maxiter = 50, #input$generations,
      run = 50
    )
    
    # Update UI with best parameters
    
    mcpars <- ga_results@solution#[1, ]
    #mcpars <- as.numeric(allpars[1,])
